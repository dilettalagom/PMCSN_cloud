...
class Msq {
  ...
  static double STOP    = 100000;        /* terminal (close the door) time */
  ...
  public static void main(String[] args) {
    ...
    while ((event[0].x != 0) || (number != 0)) {
      ...
      if (e == 0) {                                  /* process an arrival*/
        ...
        s                 = e;
        if (number >= SERVERS) {
          //System.err.println("number >= servers");
          service         = m.getService(r);
          sum[s].service += service;
          sum[s].served++;
          event[s].t      = t.current + service;
  ...
  double getArrival(Rngs r) {
     ...
     */
    r.selectStream(0);
    sarrival += exponential(1.0/12.25, r);
    return (sarrival);
  }
  double getService(Rngs r) {
     ...
     */
    r.selectStream(1);
    return (exponential(1.0/0.306, r));
  }


  double getServiceold(Rngs r) {
    /* ------------------------------
     * generate the next service time, with rate 1/6
     * ------------------------------
     */
    r.selectStream(1);
    return (uniform(2, 10, r));
  }
